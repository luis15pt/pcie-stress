...
MAINBOARD='PCIe gen5 MCIO Switch 100-Lane'
echo $MAINBOARD
export MAINBOARD

upstream_ports=$(lspci -vv -D -d 1f18:0101:0604 2>/dev/null | awk '/^[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9a-fA-F]+\.[0-9a-fA-F]+/ {dev=$1} /Express.*Upstream Port/ {print dev}')
n=0
