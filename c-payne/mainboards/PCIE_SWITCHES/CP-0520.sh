...
MAINBOARD='PCIe gen4 Switch Backplane 5x x16 - 4W'
echo $MAINBOARD
export MAINBOARD

upstream_ports=$(lspci -vv -D -d 1f18:0001:0604 2>/dev/null | awk '/^[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9a-fA-F]+\.[0-9a-fA-F]+/ {dev=$1} /Express.*Upstream Port/ {print dev}')
n=0
