...
MAINBOARD='generic c-payne switch pagewise'
echo $MAINBOARD
export MAINBOARD

upstream_ports=$(lspci -vv -D -d 1f18:*:* 2>/dev/null | awk '/^[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9a-fA-F]+\.[0-9a-fA-F]+/ {dev=$1} /Express.*Upstream Port/ {print dev}')
n=0
...
for port_up in $upstream_ports; do
  ...
  echo ''
  read -p "Press any key for next switch... " -n1 -s
done
